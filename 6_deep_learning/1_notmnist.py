...
# 首先，下载数据集到本地，数据集包含各种字母以不同形式（字体）展现在28*28像素的图片，标签是A到J，10个分类，训练集大概有500k，测试集19000标记样例，这样的大小，应该可以在任何机器上快速的训练模型
...
# 一个报告下载进度的钩子，主要是针对网速较慢的用户，下载进度每1%报告一次
...
# Problem 1
# ---------
# 让我们看看一些数据，确保它看起来是合理的。每个范例应该是一个字符A到J的图片以不同字体呈现，显示我们刚下载的样本图片。提示：你可以用IPython.display包
# Let's take a peek at some of the data to make sure it looks sensible. Each exemplar should be an image of a character A through J rendered in a different font. Display a sample of the images that we just downloaded. Hint: you can use the package IPython.display.
#
# ---

# 现在让我们以更易于管理的格式加载数据，你也许不能训练全部数据到内存中，所以我们分别加载每个类到一个数据集里，独立存储在磁盘上，稍后再将他们合并成一个可管理大小的单一数据集
# Now let's load the data in a more manageable format. Since, depending on your computer setup you might not be able to fit it all in memory, we'll load each class into a separate dataset, store them on disk and curate them independently. Later we'll merge them into a single dataset of manageable size.
# 把整个数据集转换成浮点值的三维数组（图像索引，x，y），标准化成均值为0且标准差为0.5让训练更容易
...
image_size = 28  # Pixel width and height. 像素宽和高
pixel_depth = 255.0  # Number of levels per pixel. 像素最大值
...
# Problem 2
# ---------
# 验证下数据看起来仍然不错，从nd数组中展示一个标签和图片样本，提示：可以用使用matplotlib.pyplot
...
# 按需合并和修剪训练集，根据你电脑配置可能无法训练全部数据到内存里，可以按需调整train_size。标签被储存在独立的整数数组中0-9
...
# 下一步将打乱数据，重要的是对训练和测试数据分布，进行适当的调整
...
# Problem 6
# ---------
# 了解一下现成的分类器能给你什么样的数据。检查好的东西值得学习，
# Let's get an idea of what an off-the-shelf classifier can give you on this data. It's always good to check that there is something to learn, and that it's a problem that is not so trivial that a canned solution solves it.
# 训练一个简单的模型使用50，100，1000和5000训练样本。提示：可以使用sklearn.linear_model的逻辑回归模型
# Train a simple model on this data using 50, 100, 1000 and 5000 training samples. Hint: you can use the LogisticRegression model from sklearn.linear_model.
# 可选问题：训练一个现成的模型在所有数据上
