...
gdbt=GradientBoostingClassifier(
                loss='exponential', ##损失函数默认deviance  deviance具有概率输出的分类的偏差
                learning_rate=0.1, # 默认0.1学习率
                n_estimators=300, # 迭代训练次数，回归树个数（弱学习器）
                max_depth=11, # 默认值3最大树深度
                subsample=0.6, # 子采样率，为1表示全部，推荐0.5-0.8默认1
                criterion='friedman_mse', # 判断节点是否继续分裂采用的计算方法
                min_samples_split=600, # 生成子节点所需要的最小样本数，浮点数代表百分比
                min_samples_leaf=200, # 叶子节点所需要的最小样本数，浮点数代表百分比
                min_weight_fraction_leaf=0.,
                min_impurity_split=1e-7, # 停止分裂叶子节点的阀值
                init=None,
                random_state=0, # 随机种子，方便重现，没有明白什么 
                max_features=60, # 寻找最佳分割点要考虑的特征数量auto和None全选/sqrt开方/log2对数/int自定义个数/float百分比
                verbose=0,
                max_leaf_nodes=None, # 叶子节点的个数，None不限数量
                warm_start=False, # True在前面基础上增量训练（重设参数减少训练次数） False默认值擦除重新训练
                presort='auto')  

# AUC Score (Train): 0.885425  05-88 83 
# f1_score : 0.9605
# 0.9561875
# AUC Score (Train): 0.883755  06-94 80 
# f1_score : 0.96
# 0.95625
# AUC Score (Train): 0.885578  07-83
# f1_score : 0.96075
# 0.9561875
# AUC Score (Train): 0.877621  08-86 83
# f1_score : 0.9605
# 0.95775
# AUC Score (Train): 0.890134
# f1_score : 0.963
# 0.957875
# AUC Score (Train): 0.873448  09-76
# f1_score : 0.9625
# 0.9573125
...
# train_data.info()
train_data.to_csv('data2/result.csv', index=False, header=None)  
